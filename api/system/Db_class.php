 ...
 * @author Angel Lai
 * @package Db_class
 * @copyright Copyright 2021 Db_class Project, Angel Lai.
 * @license https://opensource.org/licenses/MIT
 * @link https://Db_class.in
 */
...
class Db_classs
{
     ...
     * @return Db_class
     * @throws PDOException
     * @link https://Db_class.in/api/new
     * @codeCoverageIgnore
     */
    ...
    protected function columnPush(&$columns, array &$map, bool $root, bool $isJoin = false): string
    {
        ...
        foreach ($columns as $key => $value) {
            ...
            if (!$isIntKey && $isArrayValue && $root && count(array_keys($columns)) === 1) {
                ...
                preg_match('/(?<column>[\p{N}\p{L}\-_\.]+)(\s*\[(?<type>(String|Bool|Int|Number))\])?/u', (string)$key, $match);
    ...
    public function my_select(string $table, $join, $columns = null, $where = null): ?array
    {
        $result = [];
        foreach ($this->select($table, $join, $columns, $where) as $item) {
            $result[$item['id']] = $item;
        };
    ...
    public function my_replace($table, $columns,$where = null){
        $c = $this->count($table,$where);
        if($c>0){
            $this->update($table,$columns,$where);
        }else{
            $this->insert($table,array_merge($columns,$where));
        }
    ...
    public function has(string $table, $join, $where = null): bool
    {
        ...
        $query = $this->exec(
            $this->type === 'mssql' ?
                $this->selectContext($table, $map, $join, $column, $where, Db_class::raw('TOP 1 1')) :
